...
19 chart templates matching `prompts/chart-pick.md`. Read this file
when generating Python code for a professional chart. Each template assumes
the professional style block from `matplotlib-guide.md` is already applied.
Colors and typefaces: see `style-guide.md`.
...
import numpy as np
# (Apply rcParams from matplotlib-guide.md here)
```
