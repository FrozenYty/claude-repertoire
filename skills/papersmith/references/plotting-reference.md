...
12. **Color carrying critical info** — only color distinguishes lines, fails
   ...
   `--`, `:`, `-.`) or marker (`o`, `s`, `^`, `D`).
13. **Title set in script when caption goes in LaTeX** — duplicates info.
...
```
 ...
 9. Error bars disclosed in caption:            pass/fail
10. Title omitted (paper) or set (slides):      pass/fail
11. Lines distinguishable in B&W (linestyle):   pass/fail
12. No `jet` / `rainbow` colormap:             pass/fail
13. Saved as both .pdf and .png:                pass/fail
```
