...
- **Anti-pattern reference**: Read `references/writing-pitfalls.md` before polishing. It documents 12 common mistakes (empty adjectives, inflated cliches, machine-translation artifacts, etc.) with concrete rewrites. Use it as a checklist — if the input triggers any anti-pattern, rewrite accordingly.
...
## See also
- prompts/humanize-zh.md — After polishing, further remove AI tone and translation artifacts. Feed Part 1 [Refined Text] output as {{CHINESE_TEXT}} input to humanize-zh.
- prompts/rewrite-zh-draft.md — If the source is a fragmented draft, restructure it before sentence-level polishing.
